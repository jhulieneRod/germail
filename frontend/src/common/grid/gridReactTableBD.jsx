...
function ReactTableBD(props) {
    ...
    const fnBuscaData = () => {
        let aFilters = [];
        filters.map((filtro) => {
            if(filtro.id !== '' && filtro.value !== ''){
                aFilters.push([filtro.id, filtro.value]);
            }
        })
        const params = [pagination.pageIndex, pagination.pageSize, ...aFilters];
        props.updateListFn(params.join()).then((dados) => {
            setData(dados.data.dados);
            setPageCount(dados.data.pageCount);
            setRowCount(dados.data.rowCount);
        });
    }
    ...
    function getColumnStyle(cell) {
        ...
        if(cell.columnDef.color){
            colStyle.color = cell.columnDef.color;
        }
                                                        ...
                                                        : (
                                                            ...
                                                            >
                                                                {header.column.getCanFilter() ? (
                                                                    <div>
                                                                        {
                                                                            <Filter column={header.column} fnFiltraDados={table.getState().fnBuscaData}/>
                                                                        }
                                                                    </div>
                                                                ) : null}
                                                            </div>
...
function RowsCount({ table, loading }) {
    ...
    if (filteredRow.rows.length > 0) {
        let totalRecords = table.getState().rowCount;
...
function Pagination({ table, props, loading }) {
    ...
    return (
        <div className='pagination'>
            ...
            <BtnRefresh btnAtualizar={props.btnAtualizar} updateListFn={table.getState().fnBuscaData} />
